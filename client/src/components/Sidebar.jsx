...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  return (
    ...
    <div className={`flex flex-col h-screen min-w-72 dark:bg-gradient-to-r from-[#242124] to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 p-4 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0].content.toLowerCase().includes(search.toLowerCase())
            ...
            <div onClick={() => { navigate("/"); setSelectedChat(chat); setIsMenuOpen(false)}} key={chat._id || chat.id} className='p-2 px-4  dark:bg-[#57317C]/10 border  border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer  flex justify-between group '>
              ...
              <img src={assets.bin_icon} onClick={e=>toast.promise(deleteChat(e,chat.id))} className='hidden group-hover:block w-4 cursor-pointer not-dark:invert '/>
            </div>
          ))
        }
      ...
      {/* user account */}
      <div onClick={() => {  }} className=' flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group  '>
